...
const CheckoutPage = () => {
  ...
  const checkoutItems = useMemo(() => {
    ...
    try {
      return JSON.parse(sessionStorage.getItem('buyNowItems')) || [];
    } catch {
      return [];
    }
  }, [cartItems, isBuyNowCheckout]);
  ...
  const finishSuccessfulOrder = (orderId) => {
    if (isBuyNowCheckout) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (paymentMethod === 'razorpay') {
        ...
        finishSuccessfulOrder(order._id);
      }
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-6 sm:px-4 sm:py-8 md:py-12">
      <div className="mb-6 max-w-2xl sm:mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-950 sm:text-4xl md:text-5xl">
          Complete your order
        </h1>
      </div>

      {isBuyNowCheckout && (
        <div className="mb-5 rounded-2xl border border-gray-100 bg-white/85 p-4 text-sm text-gray-600 shadow-sm">
          Buy Now checkout is active. This order will not change your cart.
        </div>
      )}
              ...
              <div className="space-y-3">
                ...
                <label className="flex min-h-12 cursor-not-allowed items-center gap-3 rounded-xl border border-gray-200 bg-gray-100 p-3 opacity-70 sm:rounded-2xl sm:p-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Please use online payment for now.
                    </p>
                  </div>
                </label>
              </div>
